...
a pull request touches exactly the plugin it is about, and **nothing here is
binary** — the packages are release assets of whatever project publishes them,
which for these three is the app's own repository.
...
version = "1.0.0"
abi = 2
url = "https://github.com/lollipopkit/flutter_server_box/releases/download/plugins/app.serverbox.diskusage-1.0.0.sbp"
sha256 = "18602ac4…"
size = 17595
```
...
A release names either an absolute https `url` or a `path` inside the repository
— never both. These use `url`; `path` is there for a repository that would
rather carry its own packages.
...
fetched from the `url` its file named.
...
the app verifies the bytes against it. The file and the package come from
different places — this repository and a release somewhere else — so **that
digest is what binds an address to a particular set of bytes**, and it is the
same thing a Homebrew formula's `sha256` does for the tarball it points at.
...
That packs every plugin, uploads the packages to the app repository's permanent
`plugins` release, writes each plugin's file here with the digests it computed,
commits and pushes, and then fetches this repository the way a client does and
checks every version in it against what the files say.

**The packages go up before the files that name them.** A file pointing at an
address that 404s is broken for everybody who reads it; an uploaded package that
